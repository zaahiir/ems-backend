...
MAX_ROWS_PER_SHEET = 1_000_001  # 1 million (10 lakhs) records per sheet
...
class Command(BaseCommand):
            ...
            for line in data_lines:
                try:
                    ...
                    if current_amc_name and ';' in line:
                        ...
                        data.append([parsed_date.strftime('%d-%b-%Y'), current_amc_name, scheme_name, net_asset_value])
                        nav_count += 1

                except Exception as e:
                    logger.error(f"Error processing line: {line}. Error: {str(e)}")

            self.stdout.write(self.style.SUCCESS(f"Records fetched for {date_str}: {nav_count}"))
            return nav_count
    ...
    def export_to_excel(self, data, zip_file):
        ...
        sheet.append(['NAV Date', 'Fund Family', 'Fund Name', 'Net Asset Value'])
